...
import { handleInteraction } from '../../src/slack/interact';
...
function setup() {
  ...
  return { slack, attio, store, deps: { slack: slack.api, attio: attio.client, store, config, newId: () => 'sub-1' } };
}
...
describe('handleInteraction', () => {
  it('block_suggestion returns options', async () => {
    const { deps } = setup();
    const { ctx } = fakeCtx();
    const res = await handleInteraction({ type: 'block_suggestion', action_id: 'deal', value: 'coz' }, deps, ctx);
    expect(await res.json()).toEqual({ options: [{ text: { type: 'plain_text', text: 'Cozeva' }, value: 'id:d-1' }] });
  });

  it('choose_hunt swaps the chooser for the hunt form', async () => {
    const { deps, slack } = setup();
    const { ctx, flush } = fakeCtx();
    const res = await handleInteraction({ type: 'block_actions', user: { id: 'U1', name: 'sagar' }, view: { id: 'V1', private_metadata: meta }, actions: [{ action_id: 'choose_hunt', value: 'hunt' }] }, deps, ctx);
  ...
  it('edit_submission opens the stored form prefilled with the submission id in metadata', async () => {
    ...
    await handleInteraction({ type: 'block_actions', user: { id: 'U1', name: 'sagar' }, trigger_id: 'T2', channel: { id: 'C1' }, response_url: 'https://hooks/e',
      actions: [{ action_id: 'edit_submission', value: 'sub-1' }] }, deps, ctx);
    await flush();
  ...
  it('edit_submission by a non-allowlisted user is refused via response_url', async () => {
    const { deps, slack } = setup();
    const { ctx, flush } = fakeCtx();
    await handleInteraction({ type: 'block_actions', user: { id: 'U9', name: 'x' }, trigger_id: 'T', channel: { id: 'C1' }, response_url: 'https://hooks/e', actions: [{ action_id: 'edit_submission', value: 'sub-1' }] }, deps, ctx);
    await flush();
  ...
  it('view_submission with validation errors returns response_action errors', async () => {
    ...
    const res = await handleInteraction({ type: 'view_submission', user: { id: 'U1', name: 'sagar' },
      view: { id: 'V1', callback_id: 'crm_note', private_metadata: meta, state: { values: values({ record: { type: 'external_select', selected_option: null }, note: text('') }) } } }, deps, ctx);
  ...
  it('valid view_submission acknowledges immediately and runs the pipeline in the background', async () => {
    ...
    const { ctx, flush } = fakeCtx();
    const res = await handleInteraction({ type: 'view_submission', user: { id: 'U1', name: 'sagar' },
      view: { id: 'V1', callback_id: 'crm_note', private_metadata: meta, state: { values: values({ record: pick('companies:c-1', 'Cozeva (company)'), note: text('hello') }) } } }, deps, ctx);
    expect(res.status).toBe(200);
